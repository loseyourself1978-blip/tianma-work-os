...
PHASE = "Vol.8 Phase 8.2 - Implemented Function Framework Index Output Module"
BASELINE_COMMIT = "90d6d2081e7ca75de082d5447ef80e4250d8131e"
EXPECTED_RUNTIME_RECORDS = 109
...
RECORD_PATH = "records/ldd/2026-06-16/vol8_phase8_2_implemented_function_framework_index_output_module.json"
...
def main() -> int:
    ...
    require(not missing, "required_files", "all Phase 8.2 documents, schema, fixture, record, generator, and validators exist", failures)
    for path in missing:
    ...
    require(index.get("generated_for_phase") == PHASE, "phase", "generated phase matches Phase 8.2", failures)
    require(index.get("baseline_commit") == BASELINE_COMMIT, "baseline_commit", "baseline commit matches Phase 8.1 commit", failures)
    require(index.get("runtime_records") == EXPECTED_RUNTIME_RECORDS, "runtime_records", "runtime records baseline equals 109", failures)
    ...
    require(record.get("baseline_commit") == BASELINE_COMMIT and record.get("runtime_records") == EXPECTED_RUNTIME_RECORDS, "record_baseline", "runtime record preserves Phase 8.2 baseline metadata", failures)
