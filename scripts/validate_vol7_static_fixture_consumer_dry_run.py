...
def main() -> int:
    ...
    require(event_count in {101, 102, 103, 104}, "current_timeline_count", "current timeline count is baseline 101, post-dry-run 102, post-readiness-gate 103, or post-local-static-shell 104", failures)
    require(warning_count == 0, "current_timeline_warnings", "current timeline warnings remain 0", failures)
    require(latest_event in {BASELINE_TIMELINE_EVENT, DRY_RUN_TIMESTAMP, READINESS_GATE_TIMESTAMP, LOCAL_STATIC_SHELL_TIMESTAMP}, "current_latest_event", "current latest timeline event is baseline, Phase 7.2 dry-run timestamp, Phase 7.3 readiness timestamp, or Phase 7.4 local static shell timestamp", failures)
