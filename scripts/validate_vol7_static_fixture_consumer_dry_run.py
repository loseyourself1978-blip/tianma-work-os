...
def main() -> int:
    ...
    require(event_count in {101, 102, 103, 104, 105, 106, 107, 108}, "current_timeline_count", "current timeline count is baseline 101 through post-handoff 108", failures)
    require(warning_count == 0, "current_timeline_warnings", "current timeline warnings remain 0", failures)
    require(latest_event in {BASELINE_TIMELINE_EVENT, DRY_RUN_TIMESTAMP, READINESS_GATE_TIMESTAMP, LOCAL_STATIC_SHELL_TIMESTAMP, LOCAL_STATIC_SHELL_REVIEW_TIMESTAMP, LOCAL_STATIC_SHELL_DEMO_PACK_TIMESTAMP, LOCAL_STATIC_SHELL_SNAPSHOT_QA_TIMESTAMP, VOL7_HANDOFF_TIMESTAMP}, "current_latest_event", "current latest timeline event is a known Vol.7 static timeline timestamp through Phase 7.8", failures)
