...
def classify_record(path: Path, data: dict[str, Any]) -> str | None:
    ...
    if "vol6_phase6_7a_ldd_premarket_rebound_confirmation_governance_sync" in name:
        return "governance_runtime_sync"
    if "vol6_phase6_9_handoff_summary_and_vol7_readiness_gate" in name:
        return "vol6_handoff_readiness_gate"
    if "vol7_phase7_0_static_ui_shell_boundary_map" in name:
    ...
    if "vol7_phase7_2_static_fixture_consumer_dry_run_and_drift_detector" in name:
        return "governance_review"
    if "vol7_phase7_3_static_shell_implementation_readiness_gate" in name:
        return "governance_review"
    if "vol7_phase7_4_local_static_shell_skeleton_permissioned_implementation" in name:
        return "governance_review"
    if "vol7_phase7_5_local_static_shell_review_accessibility_guardrail_hardening_and_ldd_backfeed" in name:
        return "governance_review"
    if "vol7_phase7_6_local_static_shell_demo_pack_and_operator_walkthrough" in name:
        return "governance_review"
    if "vol7_phase7_7_local_static_shell_snapshot_qa_and_completion_readiness" in name:
        return "governance_review"
    ...
    if "cockpit_static_spec_integration_review" in name:
    ...
    if data.get("record_type") == "governance_review" and data.get("phase") == "Vol.6 Phase 6.9":
        return "vol6_handoff_readiness_gate"
    ...
    if data.get("record_type") == "governance_review" and str(data.get("phase", "")).startswith("Vol.7 Phase 7.2"):
        return "governance_review"
    if data.get("record_type") == "governance_review" and str(data.get("phase", "")).startswith("Vol.7 Phase 7.3"):
        return "governance_review"
    if data.get("record_type") == "governance_review" and str(data.get("phase", "")).startswith("Vol.7 Phase 7.4"):
        return "governance_review"
    if data.get("record_type") == "governance_review" and str(data.get("phase", "")).startswith("Vol.7 Phase 7.5"):
        return "governance_review"
    if data.get("record_type") == "governance_review" and str(data.get("phase", "")).startswith("Vol.7 Phase 7.6"):
        return "governance_review"
